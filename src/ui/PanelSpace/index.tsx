...
export const PanelSpace: React.FC<{
	...
	const { style } = props
	const { state } = useFormDesignContext<IFormDesignerContext>()
	...
	return (
		<Flex className={clsx(styles.panelSpace, { [styles.visible]: state.designEnable })} style={{ ...style }}>
							...
							return (
								<Panel header={el.title} key={el.title} showArrow={false}>
									<Flex gap={10} flexWrap="wrap">
										{
											el.children.length > 0 && el.children.map(item => {
												return (
													<DragItem item={item} key={item.title} />
												)
											})
										}
									</Flex>
